"""Miscellaneous utilities: NestedTensor, distributed helpers, metric logger."""
import os
import time
import pickle
import datetime
...
class MetricLogger:
    ...
    def log_every(self, iterable, print_freq, header: str = ""):
        i = 0
            ...
            if i % print_freq == 0 or i == len(iterable) - 1:
                ...
                if torch.cuda.is_available():
                    print(log_msg.format(
                        i, len(iterable), eta=eta, meters=str(self),
...
class NestedTensor:
    """Batch of images of different sizes padded together, with a mask marking padding locations."""
...
def nested_tensor_from_tensor_list(tensor_list: List[Tensor]) -> NestedTensor:
    if tensor_list[0].ndim != 3:
        raise ValueError("Expected a list of 3D tensors (C, H, W).")
